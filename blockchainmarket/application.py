...
def buy():
    ...
    if session:
        ...
        shirtsLen = len(shirts)
        # Create a transaction for blockchain
        file_path="clothing.png"
...
def prodreg():
    ...
    lname = request.form["lname"]
    email = request.form["email"]
    file = request.files['confirm']
    filename = file.filename        
    print("@@ Input posted = ", filename)
    file_path = os.path.join('./static/img/', filename)
    file.save(file_path)
   
    new = db.execute ( "INSERT INTO shirts (samplename, image, price, onSale, onSalePrice,kind,typeClothes) VALUES (:samplename, :image, :price, :onSale, :onSalePrice,:kind,:typeClothes)",
                    samplename=username, image=filename, price=password, onSale=1, onSalePrice=fname,kind=lname,typeClothes=email)
    # Render login template
    return render_template ( "product.html" )
